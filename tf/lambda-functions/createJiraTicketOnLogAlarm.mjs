...
export const handler = async function (event, context) {
  ...
  const { alarmName = '', state = {}, configuration = {} } = alarmData

  const alarmDescription = configuration?.description || ''
  ...
  const body = {
    fields: {
      ...
      summary: 'DevOps - Cloudwatch error alarm triggered',
      description: {
        version: 1,
        type: 'doc',
        content: [{ 
          ...
          content: [{ 
            type: 'text',
            text: `${alarmDescription} ${state.reason}`
          }]
        }]
      },
